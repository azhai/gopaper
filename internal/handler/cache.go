...
func (h *CacheHandler) Refresh(c fiber.Ctx) error {
	start := time.Now()

	if err := h.cache.Refresh(c.Context(), h.scanner); err != nil {
...
func (h *CacheHandler) ListDirs(c fiber.Ctx) error {
...
func (h *CacheHandler) GetSettings(c fiber.Ctx) error {
	metaPath := h.contentDir + "/_meta.toml"
	data, err := os.ReadFile(metaPath)
	if err != nil {
		return c.Status(500).JSON(model.ErrorResponse{Code: 50000, Message: "读取配置失败"})
	}
	var meta model.DirMeta
	if err := toml.Unmarshal(data, &meta); err != nil {
		return c.Status(500).JSON(model.ErrorResponse{Code: 50000, Message: "解析配置失败"})
	}
	return c.JSON(model.SuccessResponse{Code: 0, Data: meta})
}

func (h *CacheHandler) UpdateSettings(c fiber.Ctx) error {
	var meta model.DirMeta
	if err := c.Bind().JSON(&meta); err != nil {
	...
	// 刷新缓存
	if err := h.cache.Refresh(c.Context(), h.scanner); err != nil {
		return c.Status(500).JSON(model.ErrorResponse{Code: 50000, Message: "缓存刷新失败"})
	}
	return c.JSON(model.SuccessResponse{Code: 0, Message: "设置已保存并刷新缓存"})
}
