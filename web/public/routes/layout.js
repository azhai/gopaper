...
export const LayoutPage = {
    ...
    view(vnode) {
        const s = vnode.state;
        if (s.loading)
            return m('.loading-state', [m('.spinner'), m('p', '加载布局配置...')]);
        const cfg = s.config;
        return m('.layout-page', [
            ...
            m('.templates-list', cfg.templates.map((tmpl, i) => m('.card.template-card', [
                m('.template-header', {
                ...
                s.expandedIdx === i ? m('.template-body', [
                    ...
                    m('.regions-section', [
                        ...
                        m('.regions-list', tmpl.regions.map((region, j) => m('.region-row', [
                            m('input.form-control', {
                                value: region.name, placeholder: '区域标识 (如 hero)',
                                oninput: (e) => { region.name = val(e); },
                            }),
                            m('input.form-control', {
                                value: region.title, placeholder: '区域名称 (如 横幅区)',
                                oninput: (e) => { region.title = val(e); },
                            }),
