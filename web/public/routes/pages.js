            ...
            s.loading
                ...
                : filteredPages(s).length === 0
                    ...
                    : m('.table-wrap', m('table.data-table', [
                        ...
                        m('tbody', filteredPages(s).map(page => m('tr', { key: page.slug }, [
...
function dirLabel(dirs, dirPath) {
    if (!dirPath)
        return '/ (首页)';
    const d = dirs.find(x => x.dirPath === dirPath);
    return d ? `${d.title} (${d.dirPath})` : dirPath;
}
function filteredPages(s) {
    if (!s.keyword.trim())
        return s.pages;
    const kw = s.keyword.trim().toLowerCase();
    return s.pages.filter(p => p.title.toLowerCase().includes(kw) ||
        p.dirPath.toLowerCase().includes(kw));
}
async function loadPages(vnode) {
    const s = vnode.state;
    s.loading = true;
    try {
