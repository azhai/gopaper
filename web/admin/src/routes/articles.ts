...
export const ArticlesPage: m.Component<{}, ArticlesState> = {
  oninit(vnode) {
    ...
    return m('.articles-page', [
      ...
      m('.toolbar', [
        ...
        m('select.form-control', {
          ...
          value: s.dirFilter,
          onchange: (e: Event) => {
            s.dirFilter = (e.target as HTMLSelectElement).value;
            s.page = 1;
            loadArticles(vnode);
          },
        }, [
      ...
      s.loading
        ...
        : s.articles.length === 0
          ...
          : m('.table-wrap', m('table.data-table', [
              ...
              m('tbody', filteredArticles(s).map(article =>
                m('tr', { key: article.slug }, [
      ...
      totalPages > 1 ? m('.pagination', [
        ...
        ...pageRange(s.page, totalPages).map(p =>
          p === '...' ? m('span', { style: { padding: '0 6px', color: 'var(--c-text-muted)' } }, '...') :
          m('button.page-btn', {
            class: p === s.page ? 'active' : '',
            onclick: () => { s.page = p as number; loadArticles(vnode); },
          }, String(p)),
        ),
        m('button.page-btn', {
...
function filteredArticles(s: ArticlesState): ArticleSummary[] {
  if (!s.keyword.trim()) return s.articles;
...
async function loadArticles(vnode: m.Vnode<{}, ArticlesState>) {
  const s = vnode.state;
  s.loading = true;
  try {
    const result = await apiService.getArticles(s.page, s.pageSize, s.dirFilter || undefined, 'article');
    if (result.code === 0) {
      s.articles = (result.data || []) as ArticleSummary[];
