...
interface ImagesState {
  ...
  loading: boolean;
  dragging: boolean;
  lightbox: ImageInfo | null;
}

export const ImagesPage: m.Component<{}, ImagesState> = {
  oninit(vnode) {
  ...
  view(vnode) {
    ...
    return m('.images-page', [
      ...
      m('.upload-zone', {
        ...
        ondragover: (e: DragEvent) => { e.preventDefault(); s.dragging = true; },
      ...
      s.loading
        ...
        : s.images.length === 0
          ...
          : m('.image-grid', s.images.map(img =>
              m('.image-card', { key: img.fileName }, [
                m('img.image-thumb', {
                  src: img.uploadPath,
                  loading: 'lazy',
      ...
      s.lightbox ? m('.lightbox', {
        ...
        m('img', { src: s.lightbox.uploadPath, onclick: (e: Event) => e.stopPropagation() }),
        m('div', {
...
async function loadImages(vnode: m.Vnode<{}, ImagesState>) {
  vnode.state.loading = true;
  try {
    const result = await apiService.getImages(vnode.state.page, vnode.state.pageSize);
    if (result.code === 0) {
      vnode.state.images = (result.data || []) as ImageInfo[];
...
async function uploadFile(vnode: m.Vnode<{}, ImagesState>, file: File) {
